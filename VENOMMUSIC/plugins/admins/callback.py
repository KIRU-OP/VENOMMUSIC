...
from VENOMMUSIC.utils.stream.autoplay import is_autoplay_on
...
from VENOMMUSIC.utils.decorators import ActualAdminCB, languageCB
from VENOMMUSIC.utils.formatters import seconds_to_min, time_to_seconds
from VENOMMUSIC.utils.inline import close_markup
from VENOMMUSIC.utils.inline.play import colored_stream_markup, colored_stream_markup_timer, generate_progress_bar
from VENOMMUSIC.utils.colored_buttons import (
    styled_button,
        ...
        try:
            ...
            if not playlist or len(playlist) == 0:
                if await is_autoplay_on(chat_id):
                    try:
                        await VISHAL.stop_stream(chat_id)
                        from VENOMMUSIC.utils.database import remove_active_chat, remove_active_video_chat
                        await remove_active_chat(chat_id)
                        await remove_active_video_chat(chat_id)
                        from VENOMMUSIC.utils.stream.autoplay import auto_play_next
...
async def _now_playing_timer():
    ...
    """
    from VENOMMUSIC.utils.inline.play import UPDATE_INTERVAL, should_update_progress
    while True:
